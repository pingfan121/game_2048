...
namespace WindowsDemo
{
    public class CreateFileHelper
    {
        ...
        public static string BuilderCode(string tableName)
        {
            ...
            sb.Append("using Easy4net.CustomAttributes;  ").Append("\n");

            sb.Append("namespace GameDb.Logic  ").Append("\n");
            sb.Append("{  ").Append("\n");

            sb.Append("\t [Table(Name = \"").Append(tableName).Append("\")] ").Append("\n");
            sb.Append("\t public class ").Append(entityName).Append(":TbLogic\n");
            sb.Append("\t { ").Append("\n");
            ...
            foreach (TableColumn column in columns)
            {
                ...
                sb.Append("\t\t").Append("public ").Append(type).Append(" ").Append(fieldName).Append("{");
                sb.Append("\n\t\t\t").Append("get{ return _").Append(column.Name).Append(";}");
                sb.Append("\n\t\t\t").Append("set");
                sb.Append("\n\t\t\t{");
                sb.Append("\n\t\t\t\t").Append("_").Append(column.Name).Append(" = value;");

                if(column.IsPrimaryKey != "√")
                {
                    sb.Append("\n\t\t\t\t").Append("changedKeys.Add(\"").Append(fieldName).Append("\");");
                }
            ...
            //添加构造函数
            sb.Append("\t\t").Append("public ").Append(entityName).Append("()").Append("\n");
            sb.Append("\t\t{\n");
            ...
            //添加复制函数
            sb.Append("\t\t").Append("public ").Append(entityName).Append(" copy()").Append("\n");
            sb.Append("\t\t{\n");
            sb.Append("\t\t\t").Append(entityName).Append(" t = new ").Append(entityName).Append("();\n\n");
            foreach(TableColumn column in columns)
            {
                string type = TypeHelper.GetType(column.Type, column.IsNull == "√" ? true : false);
